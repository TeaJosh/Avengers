...
$fname = $lname = $email = $bio = $address = $phone = $occupation = $city = $state = "";
$pfp = "default_profile.jpg"; // Default image
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
	...
	if (empty($fname) || empty($lname) || empty($email)) {
		...
		if (isset($_FILES['pfp']) && $_FILES['pfp']['error'] == 0) {
			...
			$file_extension = strtolower(pathinfo($_FILES['pfp']['name'], PATHINFO_EXTENSION));
			...
			if (in_array($file_extension, $allowed_types)) {
				...
				if (move_uploaded_file($_FILES['pfp']['tmp_name'], $upload_path)) {
					$pfp = $new_file_name;
				} else {
		...
		if (empty($error_message)) {
			try {
				...
				$check_query = $pdo->prepare("SELECT * FROM users_info WHERE user_id = :user_id");
				$check_query->execute(['user_id' => $user_id]);

				if ($check_query->rowCount() > 0) {
                    // Update existing profile
					$stmt = $pdo->prepare("UPDATE users_info SET fname = :fname, lname = :lname, 
						email = :email, bio = :bio, address = :address, phone = :phone, 
						pfp = :pfp, occupation = :occupation
						WHERE user_id = :user_id");
				} else {
                    // Create new profile
					$stmt = $pdo->prepare("INSERT INTO users_info (user_id, fname, lname, email, bio, 
						address, phone, pfp, occupation) 
					VALUES (:user_id, :fname, :lname, :email, :bio, :address, 
						:phone, :pfp, :occupation)");
				}
				$stmt->execute([
					'user_id' => $user_id,
					'fname' => $fname,
